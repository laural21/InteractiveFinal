...
function mouseClicked(){
  ...
  for(var i = 0; i < faces.length; i++){
    if (mouseX > faces[i].xPos && mouseX < faces[i].xPos + faces[i].width && mouseY > faces[i].yPos && mouseY < faces[i].yPos + faces[i].width) {
...
function draw() {
  ...
  if (gameOn == true) {
    ...
    while (faces.length < currentFaces.length){
      ...
      faces.push(new Face(currentFaces[i].x, currentFaces[i].y, currentFaces[i].width));
    	}
    }
